...
#            self.chm[0] + 8 : {"value":   0, "fade": 0.0},
    ...
    def close(self) -> dict:
        return dict()
    def info(self) -> str:
        return "(CC0)2023 /\/asTTY\nThe driver for ADJ Mini Dekker"
    def apiver(self) -> int:
        return 1
    ...
    def set(self, param_name: str, value: int|float, fader: int|float) -> dict:
        if param_name == "color":
            self.color = value
            return {
                self.chm[0] + 1 : {"value": value // 65536,       "fade": fader},
                self.chm[0] + 2 : {"value": value % 65536 // 256, "fade": fader},
                self.chm[0] + 3 : {"value": value % 256,          "fade": fader},
            }
        else:
            return dict()
